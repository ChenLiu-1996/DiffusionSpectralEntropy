...
if __name__ == '__main__':
    ...
    for dim in tqdm(D_list):
        embeddings, labels = datasets.make_blobs(n_samples=N,n_featuresint=dim, centers=class_num, cluster_std=0.60, random_state=0)
    ...
    for dim in tqdm(D_list):
        embeddings, _ = datasets.make_blobs(n_samples=N, n_featuresint=dim, centers=1, cluster_std=0.60, random_state=0)
        num_per_class = N // num_classes
        labels = np.zeros(N)
        # Randomly assign class labels
        for i in range(1,num_classes+1):
            inds = np.random.choice(np.where(labels==0)[0], num_per_class, replace=False)
            labels[inds] = i
    ...
    for dim, gs_x, gs_y in zip([2, 3], [0, 0], [3, 4]):
        if dim == 3:
        ax = fig_mi.add_subplot(gs[gs_x, gs_y], projection='3d')
        else:
            ax = fig_mi.add_subplot(gs[gs_x, gs_y])
    ...
    for j in range(len(t_list)):
            ax.plot(D_list,
                    np.mean(separated_mi_Ys[k], axis=0),
