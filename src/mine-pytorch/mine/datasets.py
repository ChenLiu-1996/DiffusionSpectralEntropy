...
class MultivariateNormalDataset(torch.utils.data.Dataset):
    ...
    def true_entropy(self):
        d = self.cov_matrix.shape[0]  # Dimensionality
        det_cov = torch.det(self.cov_matrix)  # Determinant of covariance matrix
        entropy = 0.5 * torch.log((2 * torch.pi * torch.e)**d * det_cov)

        return entropy

    @property
    def true_X_entropy(self):
        cov = self.cov_matrix[:self.dim, :self.dim]
        d = cov.shape[0]  # Dimensionality
        det_cov = torch.det(cov)  # Determinant of covariance matrix
        entropy = 0.5 * torch.log((2 * torch.pi * torch.e)**d * det_cov)

        return entropy

class BivariateNormalDataset(torch.utils.data.Dataset):
    def __init__(self, N, X_dim, Z_dim, rho):
        self.N = N
        self.rho = rho
        self.X_dim = X_dim
        self.Z_dim = Z_dim
    ...
    def __getitem__(self, ix):
        X, Z = self.x[ix, 0:self.X_dim], self.x[ix, self.X_dim:]
        return X, Z
    ...
    @property
    def build_dist(self):
        mu = torch.zeros(self.Z_dim + self.X_dim)
        dist = MultivariateNormal(mu, self.cov_matrix)
        return dist
    ...
    def cov_matrix(self):
        '''
            Assumes that the covariance matrix is of the form:
            [[I, rho*I],
            [rho*I, I]]
            Which means that elements in X, Z are independent and
            elements in X and Z are correlated with correlation rho.
        '''
        total_dim = self.X_dim + self.Z_dim
        cov = torch.zeros((total_dim, total_dim))
        cov[torch.arange(self.X_dim), torch.arange(
            start=self.X_dim, end=total_dim)] = self.rho
        cov[torch.arange(start=self.X_dim, end=total_dim),
            torch.arange(self.X_dim)] = self.rho
        cov[torch.arange(total_dim), torch.arange(total_dim)] = 1.0
    ...
    def true_mi(self):
        '''
            MI(X, Z) = 0.5 * log( det(cov_X) * det(cov_Y) / det(cov_XY) )
            since cov_X = cov_Y = I, we get:
            MI(X, Z) = 0.5 * log(1/det(cov_XY))
        '''
        return -0.5 * np.log(np.linalg.det(self.cov_matrix.data.numpy()))
    ...
    @property
    def true_X_entropy(self):
        cov = self.cov_matrix[:self.X_dim, :self.X_dim]
        d = cov.shape[0]  # Dimensionality
        det_cov = torch.det(cov)  # Determinant of covariance matrix
        entropy = 0.5 * torch.log((2 * torch.pi * torch.e)**d * det_cov)
