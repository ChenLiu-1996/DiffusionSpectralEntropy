...
class BinaryClassifierEval(object):
    ...
    def compute_mutual_information(self, params, batcher):
        ''' Compute MI between sentence embeddings and labels '''
        # print('yo\n')
        # print(self.samples)
        # print(self.labels)
        ...
        sent_embeddings = []
        for ii in range(0, self.n_samples, params.batch_size):
            batch = self.samples[ii:ii + params.batch_size]
            embeddings = batcher(params, batch)
        ...
        csmi_sent_labels, _ = diffusion_spectral_mutual_information(
            embedding_vectors=sent_embeddings,
            reference_vectors=sent_labels,
            n_clusters=2,
            classic_shannon_entropy=True)
        
        return {'DSMI': dsmi_sent_labels, 'CSMI': csmi_sent_labels}
