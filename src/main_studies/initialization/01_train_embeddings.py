...
def train(config: AttributeHashmap) -> None:
    ...
    log_path = '%s/%s-%s-%s-ConvInitStd-%s-seed%s.log' % (
        config.log_dir, config.dataset, config.method, config.model,
        config.conv_init_std, config.random_seed)
    save_path_fig = '%s/%s-%s-%s-ConvInitStd-%s-seed%s' % (
        config.log_dir, config.dataset, config.method, config.model,
        config.conv_init_std, config.random_seed)
    ...
    model.init_params(conv_init_std=float(config.conv_init_std))
    ...
    for epoch_idx in tqdm(range(1, config.max_epoch)):
        ...
        if not (config.method == 'simclr' and skip_epoch_simlr):
            if state_dict[val_metric] > best_val_metric:
                ...
                #             '%s_%s%%.pth' % (val_metric, val_metric_pct))
                #         torch.save(best_model, model_save_path)
    ...
    save_path_numpy = '%s/%s-%s-%s-ConvInitStd-%s-seed%s/%s' % (
        config.output_save_path, config.dataset, config.method, config.model,
        config.conv_init_std, config.random_seed, 'block-results.npz')
    os.makedirs(os.path.dirname(save_path_numpy), exist_ok=True)
...
if __name__ == '__main__':
    ...
    config.conv_init_std = args.conv_init_std
    if args.random_seed is not None:
        config.random_seed = args.random_seed
    config = update_config_dirs(AttributeHashmap(config))

    # Update checkpoint dir.
    config.checkpoint_dir = '%s/%s-%s-%s-ConvInitStd-%s-seed%s/' % (
