...
from models import ResNet50, AutoEncoder
...
def get_dataloaders(
    ...
    if config.contrastive == 'NA' or config.contrastive == 'autoencoder':
...
def train(config: AttributeHashmap) -> None:
    ...
    if config.contrastive == 'autoencoder':
        channels = [config.in_channels,32,64]
        model = AutoEncoder(num_classes=config.num_classes, channels=channels,
                            code_dim=config.code_dim, imsize=config.imsize).to(device)
    else:
    ...
    if config.contrastive == 'NA':
        ...
        opt = torch.optim.AdamW(list(model.encoder.parameters()) +
                                ...
                                lr=float(config.learning_rate),
                                weight_decay=float(config.weight_decay))
    elif config.contrastive == 'autoencoder':
        opt = torch.optim.AdamW(list(model.encoder.parameters()) +
    ...
    for epoch_idx in tqdm(range(config.max_epoch)):
        ...
        for _, (x, y_true) in enumerate(train_loader):
            if config.contrastive == 'NA':
                ...
                x_recon = model.decode(code)
                #print('code; ', code.shape, x_recon.shape, x.shape)
                loss = loss_fn_recon(x, x_recon)
                state_dict['train_loss'] += loss.item() * B
        ...
        #
        '''
        Saving ckpts based on reconstruction loss
        '''
        ...
        if state_dict['train_loss'] + float(config.gap) < last_best_train_loss:
            cnt += 1
            last_best_train_loss = state_dict['train_loss']
            best_model = model.state_dict()
            model_save_path = '%s/%s-%s-train_loss_%.4f%s' % (
                config.checkpoint_dir, config.dataset, config.contrastive, state_dict['train_loss'],
                '.pth')
            torch.save(best_model, model_save_path)
            log('Best recon model (so far) successfully saved.',
                filepath=log_path,
                to_console=False)


        if cnt >= config.num_saves:
            log('Enough checkpoints saved. Ending training.',
                filepath=log_path,
                to_console=True)
            break
    return
